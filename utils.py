...
def click_followers_button(driver, logging):
    followers_button = driver.find_element(By.PARTIAL_LINK_TEXT, 'подписок')
...
def loop_unfollow(driver, logging):
    ...
    for _ in range(AMOUNT_OF_UNFOLLOW):
        ...
        follow_button = driver.find_element(By.CSS_SELECTOR, FOLLOW_BUTTON_CSS_SELECTOR)
        ...
        waiting_for_load_page(logging)
        # удаляем элемент, после того как от него отписались
        driver.execute_script('arguments[0].remove();', follow_button)
        logging.info('Элемент удален')
        waiting_for_load_page(logging, 2)
